...
export default async function AdminDashboard() {
  ...
  const [{ count: newSubmissions }, { count: newApplications }, { count: newPurchases }] = await Promise.all([
    supabase.from('contact_submissions').select('*', { count: 'exact', head: true }).eq('status', 'new'),
    supabase.from('staff_applications').select('*', { count: 'exact', head: true }).eq('status', 'new'),
    supabase.from('purchase_requests').select('*', { count: 'exact', head: true }).eq('status', 'new'),
  ]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="border border-white/10 p-6">
          <div className="font-mono text-3xl text-core-ember">{newPurchases ?? 0}</div>
          <div className="text-xs uppercase tracking-wide text-ash mt-1">New purchase requests</div>
        </div>
      </div>
      <p className="text-ash text-sm mt-10">
        Use the sidebar to edit ranks, crate keys, staff, rules, links, and the current event —
        every change is picked up by the live site immediately.
      </p>
    </div>
  );
}

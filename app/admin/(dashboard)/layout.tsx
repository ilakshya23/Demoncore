...
const NAV = [
  ...
  { href: '/admin/coins', label: 'Coin Packages' },
  { href: '/admin/purchases', label: 'Purchase Requests' },
  { href: '/admin/staff', label: 'Staff' },
  { href: '/admin/media-rank', label: 'Media Rank' },
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const validSession = await isValidSessionCookie(cookies().get(ADMIN_COOKIE)?.value);
